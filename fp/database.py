...
# ============================================================================
# V3.2 HEALTH CONTRACT MIGRATIONS
# ============================================================================

# Сохраняем оригинальные методы и добавляем миграции
...
async def _run_migrations(self) -> None:
    ...
    # Миграция 1: avg_latency в sources (только если таблица существует)
    if await self._table_exists("sources"):
        cursor = await self._conn.execute("PRAGMA table_info(sources)")
        columns = [row[1] for row in await cursor.fetchall()]
...
ProxyDatabase._column_exists = _column_exists
ProxyDatabase._table_exists = _table_exists
ProxyDatabase._run_migrations = _run_migrations

# Переопределяем __aenter__ для правильного порядка
async def __aenter__(self) -> "ProxyDatabase":
    self._conn = await aiosqlite.connect(
        str(self.db_path),
        timeout=30.0,
...
ProxyDatabase.__aenter__ = __aenter__
